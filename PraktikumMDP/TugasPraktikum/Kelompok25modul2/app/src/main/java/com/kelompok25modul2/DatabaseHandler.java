...
public class DatabaseHandler extends SQLiteOpenHelper {
    ...
    public Cursor viewDataMahasiswa(){
        SQLiteDatabase db = this.getReadableDatabase();
    ...
    public Cursor getItemID(String name){ // untuk mendapatkan ID dan Password
        SQLiteDatabase db = this.getWritableDatabase();
        String query = "SELECT " + KEY_ID + ", "+KEY_TALL+ " FROM " + TABLE_MAHASISWA +
                " WHERE " + KEY_NAME + " = '" + name + "'" ;
        Cursor data = db.rawQuery(query, null);
        return data;
    }

    //Update data menggunakan jenis data
    public boolean updateName(int id, String username, String password){
        SQLiteDatabase db = this.getWritableDatabase();

        ContentValues values = new ContentValues();
        values.put(KEY_NAME, username);
        values.put(KEY_TALL, password);

        // updating row
        db.update(TABLE_MAHASISWA, values, KEY_ID + " = ?", new String[] {
                String.valueOf(id)});
        return true;
    }
    ...
    public void deleteName(int id, String name){
        SQLiteDatabase db = this.getWritableDatabase();
        String query = "DELETE FROM " + TABLE_MAHASISWA + " WHERE "
                + KEY_ID + " = '" + id + "'" +
                " AND " + KEY_NAME + " = '" + name + "'";
        db.execSQL(query);
    }
